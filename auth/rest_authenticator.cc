...
future<> rest_authenticator::setup_reloadable_certs_credentials() {
    ...
    _certs = co_await _credentials_builder->build_reloadable_certificate_credentials([](const std::unordered_set<sstring>& files, std::exception_ptr ep) {
        if (ep) {
...
future<role_set> get_user_roles(sstring username, sstring password, const authenticator_config& conf, shared_ptr<seastar::tls::certificate_credentials> certs) {
    ...
    //tls::tls_options options = {.server_name = conf.rest_authenticator_endpoint_host};
    //auto fd = co_await tls::connect(certs, sa, options);

    std::unique_ptr<client> http_client = std::make_unique<client>(sa, std::move(certs), conf.rest_authenticator_endpoint_host);
    ...
    try {
        try {
            co_await http_client->make_request(std::move(req), seastar::coroutine::lambda([&username, &groups] (const http::reply& rep, input_stream<char> body) -> future<> {
                ralogger.debug("rest_authenticator response for user {} content_length={}", username, rep.content_length);

                sstring content = to_sstring(co_await body.read_exactly(rep.content_length));
                ...
                rjson::value json = rjson::parse(content);
                ralogger.debug("rest_authenticator response for user {} body={} json parsed", username, content);

                if (!json.IsObject()) {
                    throw exceptions::authentication_exception("Bad response from rest authenticator, json object expected");
                }

                ralogger.debug("rest_authenticator response for user {} body={} json is object", username, content);
            ...
            switch (e.status())
            {
                ...
                default:
                    throw exceptions::authentication_exception(format("Bad response from rest authenticator: {}", e.status()));
            }
        }
    }
    catch(...) {
