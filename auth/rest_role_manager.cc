...
static future<> collect_roles(cql3::query_processor &qp, std::string_view grantee_name, role_set &roles) {
    return require_record(qp, grantee_name).then([&roles](record r) {
        return do_with(std::move(r.member_of), [&roles](const role_set &memberships) {
            return do_for_each(memberships.begin(), memberships.end(), [&roles](const sstring &role_name) {
