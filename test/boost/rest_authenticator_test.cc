...
future<> with_dummy_authentication_server(std::function<void(cql_test_env&)> func, bool use_outdated_certs = false) {
    return seastar::async([func, use_outdated_certs] {
        ...
        seastar::global_logger_registry().set_logger_level("rest_authenticator", seastar::log_level::debug);
        ...
        httpd.server().invoke_on_all([b](httpd::http_server &server) {
            auto creds = b.build_server_credentials();
            server.set_tls_credentials(creds);
        }).get();

        // setup http routes
        httpd.set_routes([](httpd::routes &r) {
